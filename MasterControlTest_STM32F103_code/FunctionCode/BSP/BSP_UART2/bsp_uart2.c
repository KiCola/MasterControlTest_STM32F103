...
/* 函数体 --------------------------------------------------------------------*/ 
void Bsp_UART2_SetHandler(VoidFuncVoid func,uint8_t *USART_RX_BUF,uint16_t len)
{
...
void Bsp_UART2_Transmit(uint8_t *pData, uint16_t Size)
{
    HAL_UART_Transmit_DMA(&UARTDMAHEAD,pData, Size);  
}

/*
把此函数放到stm32f1xx_it.c文件中对应函数中，在stm32f1xx_it.c文件中须加 extern void UART2_IDLECallBack(UART_HandleTypeDef *huart);
...
{  
    if((__HAL_UART_GET_FLAG(huart,UART_FLAG_IDLE) != RESET))  
    {
        if(huart->Instance==UARTDMAHEAD.Instance)
        {
            ...
            USART2_RX_STA = USART_RX_LEN - __HAL_DMA_GET_COUNTER(huart->hdmarx);
			if(USART2_RX_STA == 0)
			{
				USART2_RX_STA = USART_RX_LEN;
			}
