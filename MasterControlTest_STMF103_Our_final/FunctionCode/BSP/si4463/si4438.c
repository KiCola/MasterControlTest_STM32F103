...
/* Public  variables ---------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const uint8_t Si4438RegTbl[] = RADIO_CONFIGURATION_DATA_ARRAY;
...
uint8_t Si4438_Config_Init(void)
{
	...
	uint32_t iCnt=0x00;
	iData=Si4438RegTbl[iCnt];
	while(iData!=0x00)
	{
		if(iData>16)return SI446X_COMMAND_ERROR;
		iCnt++;
		i++;
		if(radio_comm_SendCmdGetResp(iData, (uint8_t*)&Si4438RegTbl[iCnt], 0, 0)!=0xff) return SI446X_CTS_TIMEOUT;
		if(radio_irqLevel()==0)
		{
